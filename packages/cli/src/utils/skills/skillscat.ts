...
export interface SkillsCatAddOptions {
  force?: boolean
  repo?: boolean
  skill?: string[]
  verbose?: boolean
...
export function buildSkillsCatAddArgs(source: string, options: SkillsCatAddOptions = {}): string[] {
...
export function resolveSkillsCatEntrypoint(
  runtimeEntry: string = process.argv[1] || join(process.cwd(), 'package.json'),
): string {
...
export async function runSkillsCatAdd(
  source: string,
  options: SkillsCatAddOptions = {},
  runOptions: SkillsCatRunOptions = {},
): Promise<void> {
