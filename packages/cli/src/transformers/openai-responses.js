...
export class OpenAIResponsesTransformer {
    options;
    ...
    async formatRequest(request) {
        ...
        for (const message of request.messages || []) {
            ...
            if (message.role === 'assistant' && Array.isArray(message.tool_calls) && message.tool_calls.length > 0) {
                const formattedMessage = this.formatInputMessage(message);
                if (formattedMessage) {
    ...
    formatInputContent(content, role) {
    ...
    isResponsesPayload(payload) {
        return !!payload
            && typeof payload === 'object'
            && payload.object === 'response'
            && Array.isArray(payload.output);
    }
    convertResponseToChat(payload) {
    ...
    extractAnnotations(messageOutput) {
    ...
    extractReasoning(payload, messageOutput) {
