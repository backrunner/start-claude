import type { ExternalAuthMode, ExternalProductConfig, ExternalProductId } from './types'
...
export interface ProductInvocation {
  configName?: string
...
const nativeCommandsByProduct: Record<ExternalProductId, Set<string>> = {
  codex: new Set([
    'a',
    'app',
    'app-server',
...
export async function runExternalProductCLI(productId: ExternalProductId): Promise<void> {
  ...
  program
    ...
    .action(async (configArg: string | undefined, options: ProductProgramOptions) => {
      if (options.list) {
  ...
  program
    ...
    .action((name: string, property: keyof ExternalProductConfig, value: string) => {
  ...
  program
    ...
    .action((name: string, property?: keyof ExternalProductConfig) => {
      const config = manager.getConfig(name)
      if (!config) {
        new UILogger().error(`Configuration "${name}" not found`)
        process.exit(1)
...
async function runProductStartup(
  productId: ExternalProductId,
  rawArgs: string[],
  commanderConfigName?: string,
): Promise<void> {
  const definition = getProductDefinition(productId)
  const manager = ExternalProductConfigManager.getInstance(productId)
...
export function shouldHandleStartupDirectly(args: string[]): boolean {
  if (args.length === 0) {
    return true
  }
...
function getFirstRoutingToken(args: string[]): { type: 'delimiter' | 'native-option' | 'root' } | { type: 'positional', value: string } | undefined {
...
export function resolveProductInvocationArgs(
  args: string[],
...
function findExplicitConfigSelector(
  args: string[],
  configExists: (name: string) => boolean,
...
function findImplicitConfigSelector(
  args: string[],
  configExists: (name: string) => boolean,
  productId?: ExternalProductId,
): ConfigSelector | undefined {
...
function getNativeCommands(productId?: ExternalProductId): Set<string> {
  if (productId) {
...
function isStartConfigSelectorValue(
  value: string,
  configExists: (name: string) => boolean,
): boolean {
...
function displayConfigs(productId: ExternalProductId): void {
  const definition = getProductDefinition(productId)
...
function displayMissingConfig(productId: ExternalProductId, configName?: string): void {
  const definition = getProductDefinition(productId)
  const ui = new UILogger()
...
async function promptConfig(
  productId: ExternalProductId,
  existing?: ExternalProductConfig,
): Promise<ExternalProductConfig> {
  const definition = getProductDefinition(productId)
...
function getPassThroughArgs(args: string[], options: { selector?: ConfigSelector } = {}): string[] {
...
function redactConfig(config: ExternalProductConfig): ExternalProductConfig {
  return {
    ...config,
...
function formatSecretValue(property: keyof ExternalProductConfig, value: unknown): unknown {
...
function formatAuthMode(authMode: ExternalAuthMode): string {
  switch (authMode) {
    case 'api-key':
      return 'API key'
    case 'oauth':
