...
export function parseMarkdownFrontmatter(content) {
    ...
    const attributes = document.toJS({ mapAsMap: false });
...
export function renderMarkdownFrontmatter(content, managedAttributes) {
...
export function getFrontmatterString(attributes, key) {
    const value = attributes[key];
    return typeof value === 'string' && value.trim() ? value.trim() : undefined;
}
export function getFrontmatterStringList(attributes, key) {
    const value = attributes[key];
