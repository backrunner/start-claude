...
export async function handleAgentListCommand(options = {}) {
...
export async function handleAgentShowCommand(agentId, options = {}) {
    const ui = new UILogger(options.verbose);
    ui.displayWelcome();
    const configFile = await configManager.load();
    const library = configFile.settings.extensionsLibrary || { mcpServers: {}, skills: {}, subagents: {} };
...
export async function handleAgentAddCommand(options = {}) {
    const ui = new UILogger(options.verbose);
    ui.displayWelcome();
    const configFile = await configManager.load();
    const library = configFile.settings.extensionsLibrary || { mcpServers: {}, skills: {}, subagents: {} };
    ...
    const tools = answers.tools?.trim()
        ? answers.tools.split(',').map((t) => t.trim()).filter((t) => t.length > 0)
        : undefined;
    const subagent = {
...
export async function handleAgentEditCommand(agentId, options = {}) {
    const ui = new UILogger(options.verbose);
    ui.displayWelcome();
    const configFile = await configManager.load();
    const library = configFile.settings.extensionsLibrary || { mcpServers: {}, skills: {}, subagents: {} };
    ...
    switch (fieldAnswer.field) {
        ...
        case 'tools': {
            ...
            subagent.tools = toolsAnswer.tools?.trim()
                ? toolsAnswer.tools.split(',').map((t) => t.trim()).filter((t) => t.length > 0)
                : undefined;
            break;
        }
    }
...
export async function handleAgentDeleteCommand(agentId, options = {}) {
    const ui = new UILogger(options.verbose);
    ui.displayWelcome();
    const configFile = await configManager.load();
    const library = configFile.settings.extensionsLibrary || { mcpServers: {}, skills: {}, subagents: {} };
