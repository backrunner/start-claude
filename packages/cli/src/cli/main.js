...
import { buildClaudeArgs, buildCliOverrides, filterProcessArgs, isDebugEnabled, resolveConfig, resolveStartConfigSelectorAsync, } from './common';
...
async function handleStatusLineSync(options = {}) {
...
async function handleMcpSync(options = {}) {
    const ui = new UILogger(options.verbose);
    try {
...
program
    ...
    .option('--plugin-dir <path>', 'Load a plugin from a directory or zip', (value, previous = []) => [...previous, value])
    .option('--plugin-url <url>', 'Fetch a plugin zip from a URL', (value, previous = []) => [...previous, value])
    ...
    .option('-e, --env <key=value>', 'Set environment variable', (value, previous = []) => [...previous, value])
    ...
    const ui = new UILogger(options.verbose);
    const startConfigSelector = await resolveStartConfigSelectorAsync(process.argv.slice(2), {
    ...
    if (options.healthCheck === true) {
        ui.displayWelcome();
        const config = await resolveConfig(configManager, s3SyncManager, options, configArg, false, startConfigSelector);
        if (!config) {
            ui.error('❌ No configuration found for health check');
            process.exit(1);
        }
        if (!hasConfigApiCredentials(config)) {
            ui.error(`❌ Configuration "${config.name}" missing required endpoint information (baseUrl and apiKey or authToken)`);
            process.exit(1);
        }
    ...
    const [updateInfo, remoteUpdateResult, claudeCheckResult,] = await Promise.allSettled([
    ...
    if (shouldUseProxy) {
        await handleProxyMode(configManager, options, configArg, systemSettings, undefined, undefined, startConfigSelector);
        return;
    }
...
program
    ...
    .action(async (originalName, newName) => (await import('../commands/duplicate')).handleDuplicateCommand(originalName, newName));
program
...
mcpCmd
    ...
    .option('--env <key=value>', 'Environment variable (stdio only)', (value, previous = []) => [...previous, value])
    .option('--header <header>', 'HTTP header (http/sse only)', (value, previous = []) => [...previous, value])
    .option('--verbose', 'Enable verbose output')
    .allowUnknownOption()
    .action(async (name, args, options) => (await import('../commands/mcp')).handleMcpAddCommand(name, args, options));
mcpCmd
    .command('remove <name>')
    .description('Remove an MCP server')
    .option('--verbose', 'Enable verbose output')
    .action(async (name, options) => (await import('../commands/mcp')).handleMcpRemoveCommand(name, options));
mcpCmd
    .command('list')
    .description('List all MCP servers')
    .option('--verbose', 'Show detailed information')
    .action(async (options) => (await import('../commands/mcp')).handleMcpListCommand(options));
mcpCmd
    .command('get <name>')
    .description('Get details of a specific MCP server')
    .option('--verbose', 'Enable verbose output')
    .action(async (name, options) => (await import('../commands/mcp')).handleMcpGetCommand(name, options));
mcpCmd
    .command('add-json <name> <json>')
    .description('Add MCP server from JSON string')
    .option('--scope <scope>', 'Configuration scope: local or user', 'user')
    .option('--verbose', 'Enable verbose output')
    .action(async (name, json, options) => (await import('../commands/mcp')).handleMcpAddJsonCommand(name, json, options));
const skillCmd = program.command('skill').description('Manage skills');
skillCmd
    .command('list')
    .description('List all skills')
    .option('--verbose', 'Show detailed information')
    .action(async (options) => (await import('../commands/skill')).handleSkillListCommand(options));
skillCmd
...
skillCmd
    ...
    .option('--verbose', 'Enable verbose output')
    .action(async (skillId, options) => (await import('../commands/skill')).handleSkillEditCommand(skillId, options));
skillCmd
    .command('delete <skill-id>')
    .description('Delete a skill')
    .option('-y, --yes', 'Skip confirmation prompt')
    .option('--verbose', 'Enable verbose output')
    .action(async (skillId, options) => (await import('../commands/skill')).handleSkillDeleteCommand(skillId, options));
const agentCmd = program.command('agent').description('Manage subagents');
agentCmd
    .command('list')
    .description('List all subagents')
    .option('--verbose', 'Show detailed information')
    .action(async (options) => (await import('../commands/agent')).handleAgentListCommand(options));
agentCmd
    .command('show <agent-id>')
    .description('Show details of a specific subagent')
    .option('--verbose', 'Enable verbose output')
    .action(async (agentId, options) => (await import('../commands/agent')).handleAgentShowCommand(agentId, options));
agentCmd
    .command('add')
    .description('Add a new subagent')
    .option('--verbose', 'Enable verbose output')
    .action(async (options) => (await import('../commands/agent')).handleAgentAddCommand(options));
agentCmd
    .command('edit <agent-id>')
    .description('Edit an existing subagent')
    .option('--verbose', 'Enable verbose output')
    .action(async (agentId, options) => (await import('../commands/agent')).handleAgentEditCommand(agentId, options));
agentCmd
    .command('delete <agent-id>')
    .description('Delete a subagent')
    .option('-y, --yes', 'Skip confirmation prompt')
    .option('--verbose', 'Enable verbose output')
    .action(async (agentId, options) => (await import('../commands/agent')).handleAgentDeleteCommand(agentId, options));
...
cacheCmd
    ...
    .option('--verbose', 'Enable verbose output')
    .action(async (options) => (await import('../commands/cache')).handleCacheClearCommand(options));
cacheCmd
    .command('clear-claude')
    .description('Clear Claude installation cache only')
    .option('--verbose', 'Enable verbose output')
    .action(async (options) => (await import('../commands/cache')).handleCacheClearClaudeCommand(options));
cacheCmd
    .command('status')
