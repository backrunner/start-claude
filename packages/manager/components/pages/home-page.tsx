...
export default function HomePage({ isVSCode, initialConfigs, initialSettings }: HomePageProps): ReactNode {
  ...
  const handleDuplicate = async (config: ClaudeConfig): Promise<void> => {
    // Generate a new name with -2, -3, etc. suffix
    ...
    const newName = generateNewName(config.name)

    // Create a new config without the id (will be generated by backend)
    // Also remove isDefault and order as they should be reset for new configs
    const { id, isDefault, order, ...configWithoutId } = config
    ...
    // Directly save the duplicated config
    await saveConfig(duplicatedConfig, false, notifyConfigChange)
  }
